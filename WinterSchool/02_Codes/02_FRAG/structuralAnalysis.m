...
for nn =1: numel(NN)
    ...
    while edp<edpc %&& scale <= 2.2
          ...
          EDP(i) = edp; %#ok store the EDP fpr each time histroy analysis 
          SCALE(i) = scale; %#ok store the scale factor for each time history analysis
          disp('EDP')
          disp(edp)
          disp(' ')
    ...
    EDP_gm{nn} = EDP; %#ok store the all seqeunce of edp
    SCALE_gm{nn} = SCALE; %#ok store the all scale of EDP
    PGA_gm{nn} = max(abs(a_g)); %#ok store the all scale of EDP     
    figure(999)
    plot(EDP,SCALE);
    hold on
    clear EDP SCALE 
